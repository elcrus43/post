...
// 2. Replace MongoDB models with Supabase
const oldModel = `// Schemas
const Account = mongoose.model('Account', new mongoose.Schema({
    ...
    vkToken: String, isActive: { type: Boolean, default: true }, createdAt: { type: Date, default: Date.now }
  }));
  const Post = mongoose.model('Post', new mongoose.Schema({
    text: String, media: [Object], status: { type: String, default: 'draft' },
    scheduledAt: Date, results: [Object], createdAt: { type: Date, default: Date.now }
  }));
  const RepostRule = mongoose.model('RepostRule', new mongoose.Schema({
    source: Object, targetAccountIds: [String], filters: Object, addSourceLink: Boolean,
    appendText: String, skipDuplicates: Boolean, lastCheckedAt: Date,
  ...
  const RepostHistory = mongoose.model('RepostHistory', new mongoose.Schema({
    ruleId: mongoose.Schema.Types.ObjectId, sourceUrl: String, text: String,
    results: [Object], status: String, publishedAt: Date
  }));
  const OAuthState = mongoose.model('OAuthState', new mongoose.Schema({
    state: String, codeVerifier: String, platform: String, createdAt: { type: Date, default: Date.now, expires: 600 }
  }));`;
...
c = c.replace(oldModel, newModel);

// 3. Add Telegram proxy
